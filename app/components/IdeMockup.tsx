            ...
            <div className="rounded-xl overflow-hidden bg-[#0A0D14] border border-white/10 shadow-2xl shadow-indigo-500/10 relative z-20 flex flex-col h-[750px] font-mono text-sm text-slate-300">
                ...
                <div className="h-9 w-full bg-[#0A0D14] border-b border-white/5 flex items-center justify-between pl-4 select-none">
                    <div className="flex items-center gap-1 shrink-0">
                        <span className="text-indigo-500 text-lg font-black mr-2">❯</span>
                ...
                <div className="flex flex-1 overflow-hidden">
                    ...
                    <div className="w-12 bg-[#0A0D14] border-r border-white/5 flex flex-col items-center py-4 justify-between z-10 shrink-0">
                        <div className="flex flex-col gap-6 w-full items-center">
                            <button onClick={() => setActiveSidebar('explorer')} className={`w-full flex justify-center border-l-2 ${activeSidebar === 'explorer' ? 'text-indigo-400 border-indigo-500' : 'text-slate-500 border-transparent hover:text-slate-300'} transition-all`}>
                            ...
                            <button onClick={() => setActiveSidebar('git')} className={`w-full flex justify-center border-l-2 ${activeSidebar === 'git' ? 'text-indigo-400 border-indigo-500' : 'text-slate-500 border-transparent hover:text-slate-300'} transition-all`}>
                                <VscSourceControl className="w-5 h-5" />
                            </button>
                            <button onClick={() => setActiveSidebar('extensions')} className={`w-full flex justify-center border-l-2 ${activeSidebar === 'extensions' ? 'text-indigo-400 border-indigo-500' : 'text-slate-500 border-transparent hover:text-slate-300'} transition-all`}>
                                <VscExtensions className="w-5 h-5" />
                            </button>
                            <button onClick={() => setActiveSidebar('timeMachine')} className={`w-full flex justify-center border-l-2 ${activeSidebar === 'timeMachine' ? 'text-indigo-400 border-indigo-500' : 'text-slate-500 border-transparent hover:text-slate-300'} transition-all`}>
                                <VscHistory className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="flex flex-col gap-6 w-full items-center">
                            <button onClick={() => setIsChatOpen(!isChatOpen)} className={`w-full flex justify-center relative ${isChatOpen ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'} transition-colors`}>
                                <VscCommentDiscussion className="w-5 h-5" />
                                {isChatOpen && <div className="absolute top-0 right-2 w-2 h-2 bg-indigo-500 rounded-full shadow-[0_0_8px_rgba(99,102,241,0.8)]"></div>}
                            </button>
                            <button onClick={() => setIsTerminalOpen(!isTerminalOpen)} className={`w-full flex justify-center ${isTerminalOpen ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'} transition-colors`}>
                    ...
                    <div className="w-64 bg-[#0F131A] border-r border-white/5 shrink-0 flex flex-col ide-scrollbar overflow-y-auto">
                            ...
                            <div className="py-4">
                                ...
                                <div className="flex flex-col text-[13px] select-none">
                                    ...
                                    <div
                                        ...
                                        className={`flex items-center gap-2 pl-8 pr-4 py-1 cursor-pointer transition-colors ${activeFile === 'main.py' ? 'bg-indigo-500/10 text-indigo-300 border-l-2 border-indigo-500' : 'text-slate-400 hover:bg-white/5 border-l-2 border-transparent'}`}
                                    >
                                    ...
                                    <div
                                        ...
                                        className={`flex items-center gap-2 pl-8 pr-4 py-1 cursor-pointer transition-colors ${activeFile === 'App.jsx' ? 'bg-indigo-500/10 text-indigo-300 border-l-2 border-indigo-500' : 'text-slate-400 hover:bg-white/5 border-l-2 border-transparent'}`}
                                    >
                        ...
                        {activeSidebar === 'git' && (
                            <div className="p-4 flex flex-col h-full animate-in slide-in-from-left-4 duration-300">
                                ...
                                <textarea className="w-full bg-[#0A0D14] border border-white/10 rounded p-2 text-sm text-white resize-none outline-none focus:border-indigo-500 transition-colors h-24 mb-3 ide-scrollbar" placeholder="Message (Ctrl+Enter to commit)"></textarea>
                                <button className="w-full bg-indigo-600 hover:bg-indigo-500 text-white rounded py-1.5 text-sm font-medium transition-colors mb-6">✓ Stage files & commit</button>
                        ...
                        {activeSidebar === 'extensions' && (
                            <div className="p-4 flex flex-col h-full animate-in slide-in-from-left-4 duration-300">
                                ...
                                <div className="relative mb-4">
                                    <input type="text" defaultValue="tailwind" className="w-full bg-[#0A0D14] border border-indigo-500/50 rounded p-1.5 pl-2 text-sm text-white outline-none focus:border-indigo-400 transition-colors" />
                                </div>
                                    ...
                                    {[{ name: "Tailwind CSS Intelli...", author: "bradl", dl: "1,708,868" }, { name: "ESLint", author: "Microsoft", dl: "32,109,221" }].map((ext, i) => (
                                        <div key={i} className="flex flex-col gap-1.5 border-b border-white/5 pb-4">
                                            ...
                                            <div className="flex justify-between items-center mt-1 text-xs text-slate-500">
                                                <span>↓ {ext.dl}</span>
                                                <button className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-0.5 rounded transition-colors">Install</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        {activeSidebar === 'timeMachine' && (
                            <div className="p-4 flex flex-col h-full animate-in slide-in-from-left-4 duration-300">
                                ...
                                <div className="relative pl-4 border-l-2 border-indigo-500/30">
                                    <div className="absolute w-3 h-3 bg-indigo-500 rounded-full -left-[7px] top-1 shadow-[0_0_10px_rgba(99,102,241,0.8)]"></div>
                    ...
                    <div className="flex-1 flex flex-col bg-[#0A0D14] min-w-0">
                        ...
                        <div className="h-10 bg-[#0A0D14] border-b border-white/5 flex items-center justify-between pr-4 select-none overflow-x-auto ide-scrollbar">
                                ...
                                {Object.entries(FILES).map(([filename, data]) => (
                                    <div
                                        ...
                                        className={`flex items-center gap-2 px-4 text-[13px] border-r border-white/5 cursor-pointer transition-colors
                                            ${activeFile === filename ? 'bg-[#11151E] border-t-2 border-t-indigo-500 text-indigo-300' : 'text-slate-500 hover:bg-white/5 border-t-2 border-t-transparent'}
                                        `}
                                    >
                            ...
                            <div className="flex items-center gap-3 shrink-0">
                                ...
                                <div className="flex items-center gap-1.5 text-xs bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2 py-1 rounded cursor-pointer hover:bg-indigo-500/20 transition-colors shadow-[0_0_10px_rgba(99,102,241,0.1)]">
                            ...
                            <div className="h-64 border-t border-white/5 bg-[#0A0D14] flex flex-col shrink-0 animate-in slide-in-from-bottom-4 duration-300">
                                <div className="flex items-center justify-between px-4 h-9 border-b border-white/5 bg-[#0F131A]">
                                    <div className="flex gap-4">
                                        <div className="text-xs text-indigo-400 border-b-2 border-indigo-500 py-2 flex items-center gap-2 cursor-pointer">
                                ...
                                <div className="p-4 text-[13px] font-mono overflow-y-auto ide-scrollbar flex-1">
                                    <div className="flex items-center gap-2 text-slate-400 mb-4 bg-white/5 w-max px-3 py-1.5 rounded text-xs border border-white/5">
                                        <IoSparklesSharp className="text-indigo-400" /> Ask AI to run a command (e.g., 'kill port 3000')
                                    </div>
                        ...
                        <div className="w-80 bg-[#0F131A] border-l border-white/5 flex flex-col shrink-0 relative animate-in slide-in-from-right-8 duration-300">
                            <div className="h-12 flex items-center justify-between px-4 border-b border-white/5">
                                <div className="flex items-center gap-2 font-bold text-sm tracking-tight text-white">
                                    <span className="text-indigo-500 text-lg font-black">{'❯'}</span> VEXTOR AI
                                </div>
                            ...
                            <div className="flex-1 p-4 overflow-y-auto ide-scrollbar flex flex-col gap-4">
                                <div className="self-end bg-indigo-600 text-white px-4 py-2 rounded-xl rounded-tr-sm text-sm shadow-lg shadow-indigo-900/20 max-w-[85%]">
                                    What is React?
                                </div>
                            ...
                            {/* Dropdown Overlay */}
                            {isModelDropdownOpen && (
                                <div className="absolute bottom-16 left-4 right-4 bg-[#1e2330] border border-indigo-500/30 rounded-lg shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                                    {['Llama 3.3', 'GPT 3.5', 'Liquid AI'].map((model) => (
                                        ...
                                        >
                                            <IoSparklesSharp className={activeModel === model ? "text-indigo-400 w-4 h-4" : "text-slate-600 w-4 h-4"} /> {model}
                                        </div>
                                    ))}
                                </div>
                            )}
                            ...
                            <div className="p-3 border-t border-white/5 bg-[#0A0D14]">
                                <div className="bg-[#161a24] border border-white/10 rounded-lg flex items-center px-3 py-2 focus-within:border-indigo-500/50 transition-colors">
                                    <button
                                        onClick={() => setIsModelDropdownOpen(!isModelDropdownOpen)}
                                        className="flex items-center gap-1 text-indigo-400 hover:text-indigo-300 text-xs font-semibold pr-2 border-r border-white/10 mr-2"
                                    >
                        ...
                        <div className="bg-[#161a24] border border-white/10 rounded-xl w-[400px] shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="relative">
                                    <select className="w-full bg-[#0A0D14] border border-white/10 rounded-lg p-3 text-sm text-white appearance-none outline-none focus:border-indigo-500 cursor-pointer">
